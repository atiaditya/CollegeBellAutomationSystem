...
class CBAS(tk.Tk):
	...
	def store_all(self, periods):
		...
		for row in periods:
			...
			#print(name)
			#string = 'INSERT INTO ALLALARMS ('+ name +',' +ringtime+',' +belltype+') VALUES ({},{},{})'
			#cur.execute(string)
			cur.execute('INSERT INTO ALLALARMS (name, ringtime, belltype) VALUES (?, ?, ?)',(name, ringtime, belltype))
		cur.execute('INSERT INTO ALLALARMS_USER (name) VALUES (?)',(name,))
		conn.commit()
		conn.close()
		HomePage(self.container, self)
		self.show_frame(HomePage)

	def store_active(self,periods):
		...
		for row in periods:
			...
			#string = 'INSERT INTO ALLALARMS ('+ name +',' +ringtime+',' +belltype+') VALUES ({},{},{})'
			#cur.execute(string)
			cur.execute('INSERT INTO ACTIVE (name, ringtime, belltype) VALUES (?, ?, ?)',(name, ringtime, belltype))
		cur.execute('INSERT INTO ACTIVE_USER (name) VALUES (?)',(name,))
		conn.commit()
		conn.close()

	def retrieve_allalarms_user(self):

		conn = sqlite3.connect('CBAS.sqlite')
		cur = conn.cursor()
		cur.execute('SELECT * FROM ALLALARMS_USER')
		data = cur.fetchall()
		conn.commit()
		conn.close()
		return data
	...
	def retrieve_allalarms(self):

		conn = sqlite3.connect('CBAS.sqlite')
		cur = conn.cursor()
		cur.execute('SELECT * FROM ALLALARMS')
		data = cur.fetchall()
		conn.commit()
		conn.close()
		return data
	...
	def retrieve_active_user(self):

		conn = sqlite3.connect('CBAS.sqlite')
		cur = conn.cursor()
		cur.execute('SELECT * FROM ACTIVE_USER')
		data = cur.fetchall()
		conn.commit()
		conn.close()
		return data
	...
	def alarm(self):
		...
		active_data = self.retrieve_active()
		active_user_data = self.retrieve_active_user()
		print(active_data)
		conn = sqlite3.connect('CBAS.sqlite')
		cur = conn.cursor()
		for row in active_data:
			name,ringtime,belltype = row
			ringtime = ringtime.strip(",'")
